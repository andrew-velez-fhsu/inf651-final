...
const createElemWithText = (
  ...
  const newContent = document.createTextNode(textContext);

  if (!Object.is(className, null)) {
...
const createSelectOptions = (users) => {
  ...
  users.forEach((user) => {
    ...
    userNode.value = user.id;
    userNode.textContent = user.name;
    userOptions.push(userNode);
  });
  return userOptions;
};
...
const createComments = (comments) => {
  ...
  comments.forEach((comment) => {
    ...
    article.append(header, body, from);
...
const getUsers = async () => {
  try {
    ...
    //no requirements for this, will log a warning
...
const displayComments = async (postId) => {
  ...
  section.append(commentFragment);
  return section;
};
...
const createPosts = async (posts) => {
  ...
  for (const post of posts) {
    ...
    const postMetadata = createElemWithText("p", `Post ID: ${post.id}`);
    const author = await getUser(post.userId);
    const authorText = createElemWithText(
      "p",
      `Author: ${author.name} with ${author.company.name}`
    );
    const authorCompanyCatchphrase = createElemWithText(
      "p",
...
const displayPosts = async (posts) => {
  ...
  const defaultParagraph = document.querySelector("main p.default-text");
  const element = !posts
    ? defaultParagraph.cloneNode(true)
...
const refreshPosts = async(posts) => {
    ...
    const main = document.querySelector("main");
    const cleanMain = deleteChildElements(main);
    const frag = await displayPosts(posts);
    const addButtons = addButtonListeners();
    return [removeButtons, cleanMain, frag, addButtons];
}
